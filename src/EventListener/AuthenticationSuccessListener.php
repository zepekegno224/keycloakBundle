...
class AuthenticationSuccessListener implements EventSubscriberInterface
{
    ...
    public function onLoginSuccess(LoginSuccessEvent $event): void
    {
        ...
        $redirectAfterLogin = $session->get(KeycloakConstants::SESSION_TARGET_PATH);
        if ($redirectAfterLogin) {
            $session->remove(KeycloakConstants::SESSION_TARGET_PATH);
            $response = new RedirectResponse($redirectAfterLogin);
